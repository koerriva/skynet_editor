...
namespace GamePlay{
    void NodeEditor::Render3D() {
        ...
        DrawModel(m_Bug,m_BugPosition,1.0,DARKBROWN);
...
//        DrawGrid(10,5.0);
    ...
    Material NodeEditor::LoadMaterialPBR(Color albedo, float metalness, float roughness)
    {
        ...
        mat.maps[MAP_ALBEDO].texture = texture1;
        mat.maps[MAP_NORMAL].texture = texture1;
        mat.maps[MAP_METALNESS].texture = texture1;
        mat.maps[MAP_ROUGHNESS].texture = texture1;
        mat.maps[MAP_OCCLUSION].texture = texture1;
        ...
        SetShaderValue(mat.shader, GetShaderLocation(mat.shader, "albedo.useSampler"), &sampler, UNIFORM_INT);
        SetShaderValue(mat.shader, GetShaderLocation(mat.shader, "normals.useSampler"), &sampler, UNIFORM_INT);
        SetShaderValue(mat.shader, GetShaderLocation(mat.shader, "metalness.useSampler"), &sampler, UNIFORM_INT);
        SetShaderValue(mat.shader, GetShaderLocation(mat.shader, "roughness.useSampler"), &sampler, UNIFORM_INT);
        SetShaderValue(mat.shader, GetShaderLocation(mat.shader, "occlusion.useSampler"), &sampler, UNIFORM_INT);

        int renderModeLoc = GetShaderLocation(mat.shader, "renderMode");
        sampler = 0;
        SetShaderValue(mat.shader, renderModeLoc, &sampler, UNIFORM_INT);
